...
// 使用无锁的span信息存储
struct SpanTracker {
  std::atomic<void*> spandAddr{nullptr};
...
class CentralCache {
 ...
 private:
  ...
  std::array<std::atomic<size_t>, FREE_LIST_SIZE>
      delayCounts_;  // 每个大小类的延迟计数
  std::array<std::chrono::steady_clock::time_point, FREE_LIST_SIZE>
      lastReturnTimes_;                                   // 上次归还时间
  static const std::chrono::milliseconds DELAY_INTERVAL;  // 延迟间隔

  bool shouldPerformDelayedReturn(
